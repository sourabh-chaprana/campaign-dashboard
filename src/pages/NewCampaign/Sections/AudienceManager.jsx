...
const AudienceManager = ({
  ...
  const [filteredAttributes, setFilteredAttributes] = useState([]); // Store selected attributes
  const [searchInputValue, setSearchInputValue] = useState(""); // Track the input value for search

  // WebSocket Connection
  useEffect(() => {
    if (socketId) {
      const ws = new WebSocket(
        `ws://13.232.49.252:7040/api/dxe/websocket/websocket?id=${socketId}`
      );
  ...
  const handleAutocompleteChange = useCallback(
    (attributeCode, type) => async (event, newValue) => {
      // Update Formik
      handleChange({
        target: {
          name: attributeCode,
      ...
      if (type === "PRIMARY") {
        ...
        const updatedAttributes = [
          ...allAttributes.filter(
            (attr) =>
              !(attr.name === attributeCode && newValue.includes(attr.value))
          ),
        ];
    ...
    <form>
      <Grid container spacing={2}>
        ...
        <Grid item xs={6}>
          <Typography
            variant="h5"
            sx={{ fontWeight: 400, fontSize: "20px", color: "#525252" }}
          >
            Demographic Reach
          </Typography>
        </Grid>
        <Grid item xs={6}>
          ...
          >
            Audience Count-{" "}
            {socketValue ? socketValue : "Select the fields to reflect count"}
          </Typography>
        </Grid>
        ...
        <Grid item xs={12}>
          <Card sx={{ boxShadow: "none", border: "1px solid #3333" }}>
            <CardContent>
              {/* Typography for Secondary Attributes */}
              <Typography
                sx={{
                  fontWeight: "bold",
                  fontSize: "20px",
              ...
              <Autocomplete
                multiple
                options={secondaryAttributes} // All available attributes
                getOptionLabel={(option) => option.attributeName}
                value={filteredAttributes} // Store selected attributes
                onChange={(event, newValue) => setFilteredAttributes(newValue)} // Handle multi-select
                inputValue={searchInputValue} // Controlled input value for search
              ...
              {loadingSecondary ? (
                ...
                // Render multi-select fields based on the selected attributes in the search bar
                filteredAttributes.map((attribute) => (
                  <Grid item xs={12} key={attribute.id} mt={2}>
                    ...
                    <Autocomplete
                      ...
                      onOpen={() =>
                        fetchSecondaryOptions(attribute.attributeCode)
                      } // Fetch options for the attribute
                      renderInput={(params) => (
                        <TextField
                          {...params}
                          variant="outlined"
        ...
        {/* <Grid item xs={12}>
          ...
          <Typography sx={{ fontWeight: "bold", fontSize: "20px", marginY: 2 }}>
            {" "}
            Additional Parameters
          </Typography>
